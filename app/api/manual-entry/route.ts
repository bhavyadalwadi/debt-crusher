...
export async function POST(request: Request) {
  try {
    const input = entry.parse(await request.json());
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (input.entity === "cash") {
        ...
        const before = input.id ? await tx.cashAccount.findUnique({ where: { id } }) : null;
        ...
        const before = input.id ? await tx.creditCard.findUnique({ where: { id } }) : null;
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Manual entry failed" }, { status: 400 });
  }
}
